...
class _InitCoursePageState extends State<InitCoursePage> {
  ...
  Future<void> _recognizeFace() async {
    try {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse('http://192.168.100.190:5000/check_face'),
      );
  ...
  Future<void> _registerAttendance(String nombreStudent) async {
    try {
      ...
      var url = Uri.parse('http://192.168.100.190/ProyectoFlutter/appregister/lib/Backend/registrate_attendance.php'); // Asegúrate de que la URL sea correcta
    ...
    return await showDialog<bool>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('¿Desea registrar la asistencia para el estudiante $nombreStudent en el curso con ID $courseId?'),
          actions: [
            TextButton(
              onPressed: () {
